...
async function main() {
	try {
		...
		execSync(`pnpm cli ${TEST_PROJECT_DIR} --timeout 5000 --expect-text Hello`, { stdio: 'inherit' });

		console.log('Test 2: Scenario file test');
		execSync(`pnpm cli ${TEST_PROJECT_DIR} --scenario test-scenario.yaml --timeout 15000`, { stdio: 'inherit' });
